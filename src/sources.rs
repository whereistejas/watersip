...
#[async_trait::async_trait]
impl<'a> Source<'a> for ExchangeRatesApi {
    const IMAGE: &'a str = "airbyte/source-exchange-rates";
    fn specs(&self) -> serde_json::Value {
        serde_json::Value::String(r#" {"spec":{"connectionSpecification":{"$schema":"http://json-schema.org/draft-07/schema#","additionalProperties":false,"properties":{"access_key":{"airbyte_secret":true,"description":"Your API Access Key. See <a href=\"https://exchangeratesapi.io/documentation/\">here</a>. The key is case sensitive.","type":"string"},"base":{"description":"ISO reference currency. See <a href=\"https://www.ecb.europa.eu/stats/policy_and_exchange_rates/euro_reference_exchange_rates/html/index.en.html\">here</a>. Free plan doesn't support Source Currency Switching, default base currency is EUR","examples":["EUR","USD"],"type":"string"},"start_date":{"description":"Start getting data from that date.","examples":["YYYY-MM-DD"],"pattern":"^[0-9]{4}-[0-9]{2}-[0-9]{2}$","type":"string"}},"required":["start_date","access_key"],"title":"ratesapi.io Source Spec","type":"object"},"documentationUrl":"https://docs.airbyte.io/integrations/sources/exchangeratesapi"},"type":"SPEC"} "#.to_string())
    }
}
